...
module Rapid7VmConsole
  ...
  class ReportFrequency
    # The next scheduled run-times for generation of the report when type is `schedule`.
    attr_accessor :next_runtimes

    # How often the report generates when type is `schedule`.
    attr_accessor :repeat

    # When the report starts generating when type is `schedule`.
    attr_accessor :start
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('String', ['schedule', 'scan', 'none'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ['schedule', 'scan', 'none'])
    ...
    def hash
      [next_runtimes, repeat, start, type].hash
    end
