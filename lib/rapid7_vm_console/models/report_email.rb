...
module Rapid7VmConsole
  ...
  class ReportEmail
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'additional')
        self.additional = attributes[:'additional']
      end

      if attributes.has_key?(:'additionalRecipients')
        if (value = attributes[:'additionalRecipients']).is_a?(Array)
          self.additional_recipients = value
        end
      end

      if attributes.has_key?(:'assetAccess')
        self.asset_access = attributes[:'assetAccess']
      end
    ...
    def valid?
      access_validator = EnumAttributeValidator.new('String', ["file", "zip", "url", "none"])
      return false unless access_validator.valid?(@access)
      additional_validator = EnumAttributeValidator.new('String', ["file", "zip", "none"])
      return false unless additional_validator.valid?(@additional)
      owner_validator = EnumAttributeValidator.new('String', ["file", "url", "zip", "none"])
    ...
    def access=(access)
      validator = EnumAttributeValidator.new('String', ["file", "zip", "url", "none"])
    ...
    def additional=(additional)
      validator = EnumAttributeValidator.new('String', ["file", "zip", "none"])
      unless validator.valid?(additional)
        fail ArgumentError, "invalid value for 'additional', must be one of #{validator.allowable_values}."
      end
      @additional = additional
    end
    ...
    def owner=(owner)
      validator = EnumAttributeValidator.new('String', ["file", "url", "zip", "none"])
    ...
    def hash
      [access, additional, additional_recipients, asset_access, owner, smtp].hash
    end
