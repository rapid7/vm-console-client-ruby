=begin
Ruby InsightVM API Client

OpenAPI spec version: 3
Contact: support@rapid7.com
Generated by: https://github.com/swagger-api/swagger-codegen.git
Swagger Codegen version: 2.4.0

=end

require 'date'

module Rapid7VmConsole
  # 
  class ScanTemplateDiscoveryPerformance
    # The number of packets to send per second during scanning.
    attr_accessor :packet_rate

    # The number of discovery connection requests to be sent to target host simultaneously. These settings has no effect if values have been set for `scanDelay`.
    attr_accessor :parallelism

    # The maximum number of attempts to contact target assets. If the limit is exceeded with no response, the given asset is not scanned. Defaults to `3`.
    attr_accessor :retry_limit

    # The duration to wait between sending packets to each target host during a scan.
    attr_accessor :scan_delay

    # The duration to wait between retry attempts.
    attr_accessor :timeout

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'packet_rate' => :'packetRate',
        :'parallelism' => :'parallelism',
        :'retry_limit' => :'retryLimit',
        :'scan_delay' => :'scanDelay',
        :'timeout' => :'timeout'
      }
    end

    # Attribute type mapping.
    def self.swagger_types
      {
        :'packet_rate' => :'ScanTemplateDiscoveryPerformancePacketsRate',
        :'parallelism' => :'ScanTemplateDiscoveryPerformanceParallelism',
        :'retry_limit' => :'Integer',
        :'scan_delay' => :'ScanTemplateDiscoveryPerformanceScanDelay',
        :'timeout' => :'ScanTemplateDiscoveryPerformanceTimeout'
      }
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      # convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h| h[k.to_sym] = v }

      if attributes.has_key?(:'packetRate')
        self.packet_rate = attributes[:'packetRate']
      end

      if attributes.has_key?(:'parallelism')
        self.parallelism = attributes[:'parallelism']
      end

      if attributes.has_key?(:'retryLimit')
        self.retry_limit = attributes[:'retryLimit']
      end

      if attributes.has_key?(:'scanDelay')
        self.scan_delay = attributes[:'scanDelay']
      end

      if attributes.has_key?(:'timeout')
        self.timeout = attributes[:'timeout']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array.new
      if !@retry_limit.nil? && @retry_limit > 15
        invalid_properties.push('invalid value for "retry_limit", must be smaller than or equal to 15.')
      end

      if !@retry_limit.nil? && @retry_limit < 1
        invalid_properties.push('invalid value for "retry_limit", must be greater than or equal to 1.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      return false if !@retry_limit.nil? && @retry_limit > 15
      return false if !@retry_limit.nil? && @retry_limit < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] retry_limit Value to be assigned
    def retry_limit=(retry_limit)
      if !retry_limit.nil? && retry_limit > 15
        fail ArgumentError, 'invalid value for "retry_limit", must be smaller than or equal to 15.'
      end

      if !retry_limit.nil? && retry_limit < 1
        fail ArgumentError, 'invalid value for "retry_limit", must be greater than or equal to 1.'
      end

      @retry_limit = retry_limit
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          packet_rate == o.packet_rate &&
          parallelism == o.parallelism &&
          retry_limit == o.retry_limit &&
          scan_delay == o.scan_delay &&
          timeout == o.timeout
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Fixnum] Hash code
    def hash
      [packet_rate, parallelism, retry_limit, scan_delay, timeout].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      self.class.swagger_types.each_pair do |key, type|
        if type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the the attribute
          # is documented as an array but the input is not
          if attributes[self.class.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[self.class.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[self.class.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[self.class.attribute_map[key]]))
        end # or else data not found in attributes(hash), not an issue as the data can be optional
      end

      self
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def _deserialize(type, value)
      case type.to_sym
      when :DateTime
        DateTime.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :BOOLEAN
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        temp_model = Rapid7VmConsole.const_get(type).new
        temp_model.build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        next if value.nil?
        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end
  end
end
